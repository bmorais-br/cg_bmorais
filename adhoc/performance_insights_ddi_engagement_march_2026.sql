...
-- 2: Change table creation locations (find + replace): sandbox_cpaoletti.competitive_intelligence
-- database + schema
...
set performance_sent_dt = '2026-03-05'; 
...
CREATE OR REPLACE TABLE sandbox_cpaoletti.competitive_intelligence.PERFORMANCE_HEALTH_DEALER_LIST AS
select 
...
CREATE OR REPLACE TABLE
  sandbox_cpaoletti.competitive_intelligence.NBDR_LISTING_PRICE_AT_SEND_BY_COUNTRY_MAR_03_05 AS
...
WITH
    cte_emails_and_listings AS (
        ...
        FROM
            warehouse_prod_fivetran.site_history.sent_email s
            JOIN warehouse_prod_fivetran.site_history.sent_email_context c_l_id ON c_l_id.sent_email_id = s.id
            AND s.email_type IN ('NEXT_BEST_DEAL_RATING', 'NEXT_BEST_DEAL_RATING_CA', 'NEXT_BEST_DEAL_RATING_UK')
            AND c_l_id.context_key = 'l_id'
            JOIN warehouse_prod_fivetran.site_history.sent_email_context c_sp_id ON c_sp_id.sent_email_id = s.id
            AND s.email_type IN ('NEXT_BEST_DEAL_RATING', 'NEXT_BEST_DEAL_RATING_CA', 'NEXT_BEST_DEAL_RATING_UK')
            AND c_sp_id.context_key = 'sp_id'
            JOIN sandbox_cpaoletti.competitive_intelligence.PERFORMANCE_HEALTH_DEALER_LIST as performance_health_dealers
...
-- flag as instances where the NBDR email price was lowered and the deal rating was changed as a result
 CREATE OR REPLACE TABLE
sandbox_cpaoletti.competitive_intelligence.NBDR_CHANGES_FLAGGED_BY_COUNTRY_MAR_03_05 AS
WITH
    cte_dats AS (
        SELECT
        ...
        FROM
            ...
            and denormalized_predictions.country = 'GB' -- IMV2 only applicable for UK 
            AND denormalized_predictions.country = imv_prod.country_code
            JOIN sandbox_cpaoletti.competitive_intelligence.NBDR_LISTING_PRICE_AT_SEND_BY_COUNTRY_MAR_03_05 AS dts ON dts.service_provider_id = imv_prod.service_provider_id
...
CREATE OR REPLACE TABLE sandbox_cpaoletti.competitive_intelligence.NBDR_CHANGES_FLAGGED_BY_COUNTRY_W_PERSISTENCE_MAR_03_05 AS 
WITH
    cte_filtered AS (
        SELECT
            service_provider_id
        ...
        FROM
            sandbox_cpaoletti.competitive_intelligence.NBDR_CHANGES_FLAGGED_BY_COUNTRY_MAR_03_05
        WHERE
...
SELECT
   ...
   , count(*) as row_ctr
FROM
    sandbox_cpaoletti.competitive_intelligence.NBDR_CHANGES_FLAGGED_BY_COUNTRY_MAR_03_05 AS dt1
...
from sandbox_cpaoletti.competitive_intelligence.NBDR_CHANGES_FLAGGED_BY_COUNTRY_W_PERSISTENCE_MAR_03_05 as dats
