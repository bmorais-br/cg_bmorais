# AN-10866 Dashboard Modularization Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Split `AN-10866.py` (910 lines) into `config.py`, `queries.py`, `components.py`, and a slim entrypoint `AN-10866.py`, preserving all functions and behavior exactly.

**Architecture:** Constants live in `config.py` (no imports). All SQL and `@st.cache_data` data-loaders live in `queries.py` (no UI calls). Reusable Streamlit widgets live in `components.py`. The main file imports from all three and handles only page config, sidebar, data loading, and tab rendering.

**Tech Stack:** Python 3.10+, Streamlit, Pandas, Altair, Snowflake Snowpark. All files co-located in the same Snowflake stage — standard relative imports work.
...
```bash
python3 -c "from config import DEALER_LIST, C_BLUE, C_GREEN, C_AMBER, C_PURPLE, ACCEPT_STAFF; print('OK')"
```
Expected: `OK`
...
```bash
git add streamlit/config.py
git commit -m "feat: extract constants into config.py"
```

---

### Task 2: Create `queries.py`

**Files:**
- Create: `streamlit/queries.py`

- [ ] **Step 1: Create the file**

`streamlit/queries.py` — copy every data-access function from `AN-10866.py` verbatim, adding imports at the top and the `DEALER_LIST` reference from `config`:
...
- [ ] **Step 2: Verify imports parse cleanly (Snowpark not needed for import check)**

```bash
cd /Users/bmorais/cg_bmorais/streamlit && python3 -c "
import ast, sys
...
```bash
git add streamlit/queries.py
git commit -m "feat: extract data-access functions into queries.py"
```

---

### Task 3: Create `components.py`

**Files:**
- Create: `streamlit/components.py`
...
```bash
git add streamlit/components.py
git commit -m "feat: extract UI helper functions into components.py"
```

---

### Task 4: Rewrite `AN-10866.py` as the slim entrypoint

**Files:**
- Modify: `streamlit/AN-10866.py`

- [ ] **Step 1: Replace the entire file contents**

`streamlit/AN-10866.py`:
```python
import streamlit as st
import pandas as pd
from snowflake.snowpark.context import get_active_session
...
- [ ] **Step 2: Verify syntax of the rewritten entrypoint**

```bash
cd /Users/bmorais/cg_bmorais/streamlit && python3 -c "
import ast
with open('AN-10866.py') as f:
    src = f.read()
ast.parse(src)
print('syntax OK')
"
...
- [ ] **Step 3: Verify all three new modules also parse cleanly**

```bash
cd /Users/bmorais/cg_bmorais/streamlit && python3 -c "
import ast
for fname in ['config.py', 'queries.py', 'components.py']:
...
```bash
git add streamlit/AN-10866.py
git commit -m "refactor: slim down AN-10866.py to entrypoint — imports from config/queries/components"
```
